...
- 输出：判定为不匹配的 skill 目录会被直接删除；判定结果缓存到 `filter_result.json`（可通过配置 `filter_result_cache` 固定到如 `exps/skill_discovery/results/filter_domain_result.json`），支持断点续跑
