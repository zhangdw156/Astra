...
PROMPT_LIGHT_PATH = WORKFLOW_DIR / "prompts" / "skill_to_tools_jsonl.md"
...
# light 模式输出目录：仅生成 tools.jsonl 时落盘到此目录
SYNTHESIZED_ENVS_ROOT = WORKFLOW_DIR / "synthesized_envs"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--light",
        action="store_true",
        help="Light mode: generate only tools.jsonl at <env-dir>/tools.jsonl (no mcp_server, docker, etc.)",
    )
...
def main() -> int:
    ...
    if light:
        # Light 模式：仅生成 tools.jsonl，输出到 synthesized_envs/<skill_id>/tools.jsonl
        env_dir = SYNTHESIZED_ENVS_ROOT / skill_dir.name
        env_dir.mkdir(parents=True, exist_ok=True)
        output_path = env_dir / "tools.jsonl"
        prompt_path = PROMPT_LIGHT_PATH
        if not prompt_path.exists():
            print(f"Error: Light prompt not found: {prompt_path}")
            return 1
        ...
        prompt_text = prompt_text.replace("{OUTPUT_PATH}", str(output_path))
        task_prefix = (
            "Generate **only** the file tools.jsonl for the light data-synthesis pipeline.\n\n"
            f"- Skill dir: {skill_dir}\n"
            f"- Output file (write exactly this file): {output_path}\n\n"
            "STEP 2 — Validate (required): After writing the file, run:\n"
            f"  uv run python {validate_script} {env_dir}\n"
            "The validator will check that tools.jsonl exists and each line is valid JSON. Fix any failures until exit code 0.\n\n"
            "---\n\n"
        )
        task_text = task_prefix + prompt_text
        print("=" * 60)
        print("OpenCode Environment Generation (Light: tools.jsonl only)")
        print("=" * 60)
