...
# API Configuration
TEMPLATE_ID = "82165150735472749"
API_BASE_URL = "https://api.browseract.com/v2/workflow"

def run_youtube_search_task(api_key, keywords, video_type="Videos", limit=100):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python youtube_search_api.py <keywords> [video_type] [limit]", flush=True)
        sys.exit(1)
    ...
    keywords = sys.argv[1]
    video_type = sys.argv[2] if len(sys.argv) > 2 else "Videos"
    limit = sys.argv[3] if len(sys.argv) > 3 else 100
    
    result = run_youtube_search_task(api_key, keywords, video_type, limit)
    if result:
